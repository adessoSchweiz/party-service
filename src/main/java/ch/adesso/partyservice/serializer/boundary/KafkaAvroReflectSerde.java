package ch.adesso.partyservice.serializer.boundary;
...
public class KafkaAvroReflectSerde<T> implements Serde<T> {
	...
	public KafkaAvroReflectSerde(SchemaRegistryClient client, Map<String, ?> props) {
		inner = Serdes.serdeFrom(new KafkaAvroReflectSerializer<T>(client, props),
				new KafkaAvroReflectDeserializer<T>(client, props));
	}
