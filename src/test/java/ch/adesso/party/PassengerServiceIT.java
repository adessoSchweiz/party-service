...
import ch.adesso.party.entity.Address;
import ch.adesso.party.entity.CreditCard;
...
public class PassengerServiceIT {
	...
	public void a01_shouldCreatePassenger() {
		JsonObject personToCreate = Json.createObjectBuilder()
				...
				.add(Passenger.JSON_KEYS.STATUS.getKeyName(), STATUS).add(Passenger.JSON_KEYS.MOBIL.getKeyName(), MOBIL)
				.add(Passenger.JSON_KEYS.EMAIL.getKeyName(), EMAIL)
				.add(Passenger.JSON_KEYS.ADDRESS.getKeyName(),
						Json.createObjectBuilder().add(Address.JSON_KEYS.STREET.getKeyName(), ADDRESS.STREET)
				...
				.add(Passenger.JSON_KEYS.CREDIT_CARD.getKeyName(),
		...
		Response postResponse = this.personProvider.target().request().post(Entity.json(personToCreate));

		assertThat(postResponse.getStatus(), is(201));

		ID = postResponse.readEntity(JsonObject.class).getString(Passenger.JSON_KEYS.ID.getKeyName());
	}

	@Test
	public void a02_shouldReturnPassengerForHealthCheck() throws InterruptedException {
		JsonObject passenger = this.healthProvider.target().queryParam("personId", ID)
